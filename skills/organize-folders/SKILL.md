...
沙盒整理模式下，如果遇到大文件或整个文件夹，默认不要复制内容本体。优先在沙盒中创建指向原路径的链接或路径占位说明，等用户确认正式实施后，再决定是否真实复制、移动或归档。
...
- 大文件、大目录和高价值资料默认使用路径链接或路径占位，不做实体复制。
- 链接仅用于预览新结构，不代表文件已经完成迁移。
- 如果系统不支持创建原生链接，生成小型路径说明文件作为占位。
- 用户确认正式实施前，不要把链接占位替换为复制或移动。
