...
当用户主动提到或实际拥有多个存储介质，例如本机磁盘、NAS、移动硬盘、云盘或代码托管平台时，可使用“存储拓扑模型”作为可选建模方式，明确每个介质的职责、主副本位置、备份副本位置和同步方向，再设计目录结构。不要默认假设用户拥有 NAS、移动硬盘或代码托管平台。
...
多词英文目录默认使用下划线连接，避免空格和短横线。例如使用 `02.system_tools`，不要使用 `02.system-tools`。如果用户已有稳定命名习惯，优先沿用并在规则中说明。

顶级目录和次级目录默认都要放置一个 `目录说明.txt`。说明文件用于告诉人和 Agent：这个目录的用途、放什么、不放什么、维护周期和风险边界。项目仓库、系统目录、应用安装目录、虚拟磁盘目录、同步工具强绑定目录可以例外，避免破坏工具链或制造无关未跟踪文件。
...
整理完成后必须输出新目录说明表，说明每个目录的中文名称、用途、放什么、不放什么和维护周期。同时，在用户确认执行真实整理后，应为受管理的顶级目录和次级目录创建或更新 `目录说明.txt`；不适合写入说明文件的目录要在日志中说明跳过原因。
